...
interface ExamListProps {
  ...
  onCardAction: (examId: string) => void
  emptyMessage: string
}

export function ExamList({ variant, onCardAction, emptyMessage }: ExamListProps) {
            ...
            {filtered.map((exam) =>
              variant === "employer" ? (
                <ExamCard
                  ...
                  variant="employer"
                  exam={exam}
                  onViewCandidates={() => onCardAction(exam.id)}
                />
              ) : (
                <ExamCard
                  key={exam.id}
