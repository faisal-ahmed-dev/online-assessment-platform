...
export function CandidateSignIn() {
  const { signIn } = useAuth(UserRole.Candidate)
        ...
        <div className="w-full max-w-[571px]">
          ...
          <div className="bg-white rounded-2xl border border-[#E5E7EB] p-8 shadow-sm">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              <div>
                ...
                <div className="flex justify-end mt-1">
                  <button type="button" className="text-xs text-[#6C3AE8] hover:underline">
                    Forget Password?
                  </button>
                </div>
              </div>
            ...
            <p className="text-center text-xs text-[#9CA3AF] mt-4">
              Demo: <span className="font-mono">{MOCK_CANDIDATE_CREDENTIALS.email}</span> /{" "}
              <span className="font-mono">{MOCK_CANDIDATE_CREDENTIALS.password}</span>
            </p>
          </div>
        </div>
      </main>
